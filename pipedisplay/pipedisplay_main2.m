...
%     p3 = p3.*stimTexVar./std(p3(:))+currCol(3);
    p1 = p1.*stimTexVar./std(p1(:))+0.25+0.5*currCol(1);
    p2 = p2.*stimTexVar./std(p2(:))+0.25+0.5*currCol(2);
    p3 = p3.*stimTexVar./std(p3(:))+0.25+0.5*currCol(3);
    ...
    Screen('FillOval', w1, max(255*currCol(1)-flickint*flick,0), [0.5*[1024,768]-disksize,0.5*[1024,768]+disksize]);
    vbl1 = Screen('Flip', w1, vbl1+1/20-0.25/60);
    Screen('DrawTexture', w2, tex2, [], [0,0,resMon.width,resMon.height]);
    Screen('FillOval', w2, max(255*currCol(2)-flickint*flick,0), [0.5*[1024,768]-disksize,0.5*[1024,768]+disksize]);
    vbl2 = Screen('Flip', w2, vbl2+1/20-0.25/60);
    Screen('DrawTexture', w3, tex3, [], [0,0,resMon.width,resMon.height]);
    Screen('FillOval', w3, max(255*currCol(3)-flickint*flick,0), [0.5*[1024,768]-disksize,0.5*[1024,768]+disksize]);
    ...
    Screen('FillOval', w0, max(255*currCol(1)-flickint*flick,0), [0.5*resSup.width/3-disksize/3,0.5*resSup.height/3-disksize/3,0.5*resSup.width/3+disksize/3,0.5*resSup.height/3+disksize/3]);
    Screen('FillOval', w0, max(255*currCol(2)-flickint*flick,0), [1.5*resSup.width/3-disksize/3,0.5*resSup.height/3-disksize/3,1.5*resSup.width/3+disksize/3,0.5*resSup.height/3+disksize/3]);
    Screen('FillOval', w0, max(255*currCol(3)-flickint*flick,0), [2.5*resSup.width/3-disksize/3,0.5*resSup.height/3-disksize/3,2.5*resSup.width/3+disksize/3,0.5*resSup.height/3+disksize/3]);
