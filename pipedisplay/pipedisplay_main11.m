...
while 1
    ...
    for ww=1:3
        ...
        Screen('FillOval', stimwin(ww), max(255*currAvail(ww)-flickint*flick,0), [0.5*[1024,768]-disksize,0.5*[1024,768]+disksize]);
        ...
        Screen('FillOval', w0, max(255*currAvail(ww)-flickint*flick,0), [(ww-0.5)*resSup.width/3-disksize/3,0.5*resSup.height/3-disksize/3,(ww-0.5)*resSup.width/3+disksize/3,0.5*resSup.height/3+disksize/3]);
        if currAvail(ww) && displaystim
...
%                 fprintf('\nBox %i: lambda=%f, avail=%i',temp(1), currLambda(temp(1)), currAvail(temp(1)));
            end
            tline = fgetl(fid);
        end
        fclose(fid);
